const AWS = require("aws-sdk");
const lambda = new AWS.Lambda();

const _ = require("lodash");
const retry = require("async-retry");

const bailIfErrorNotRetryable = (bail) => (error) => {
	if (!error.retryable) {
		bail(error);
	} else {
		throw error;
	}
};

const getRetryConfig = (onRetry) => (
	{
		retries: parseInt(process.env.RETRIES || "3"),
		minTimeout: parseFloat(process.env.RETRY_MIN_TIMEOUT || "5000"),
		maxTimeout: parseFloat(process.env.RETRY_MAX_TIMEOUT || "60000"),
		factor: 2,
		onRetry
	}
);

/**
 * List lambda functions
 */
const listFunctions = async () => {
	console.info("listing all available functions...");

	const loop = async (acc = [], marker) => {
		const params = {
			Marker: marker,
			MaxItems: 10
		};

		const res = await retry(
			(bail) => lambda
				.listFunctions(params)
				.promise()
				.catch(bailIfErrorNotRetryable(bail)),
			getRetryConfig((err) => {
				console.warn("retrying listFunctions after error...", err);
			}));
		const functions = res.Functions.map(x => x.FunctionArn);
		const newAcc = acc.concat(functions);

		if (res.NextMarker) {
			return loop(newAcc, res.NextMarker);
		} else {
			// Shuffle newAcc array
			console.info(`found ${newAcc.length} functions`, { count: newAcc.length });
			return newAcc.sort(() => Math.random() - Math.random());
		}
	};

	return loop();
};

/**
 * List lambda versions
 * @param funcArn Lambda function arn 
 */
const listVersions = async (funcArn) => {
	console.debug("listing versions...", { function: funcArn });

	const loop = async (acc = [], marker) => {
		const params = {
			FunctionName: funcArn,
			Marker: marker,
			MaxItems: 20
		};

		const res = await retry(
			(bail) => lambda
				.listVersionsByFunction(params)
				.promise()
				.catch(bailIfErrorNotRetryable(bail)),
			getRetryConfig((err) => {
				console.warn("retrying listVersionsByFunction after error...", { function: funcArn }, err);
			}));
		const versions = res.Versions.map(x => x.Version).filter(x => x !== "$LATEST");
		const newAcc = acc.concat(versions);

		if (res.NextMarker) {
			return loop(newAcc, res.NextMarker);
		} else {
			console.debug("found versions [NOT $LATEST]", { versions: newAcc.join(",") });
			return newAcc;
		}
	};

	return loop();
};

/**
 * List aliased versions
 * @param funcArn Lambda function arn
 * @returns list of aliased lambda versions
 */
const listAliasedVersions = async (funcArn) => {
	console.debug("listing aliased versions...", { function: funcArn });

	const loop = async (acc = [], marker) => {
		const params = {
			FunctionName: funcArn,
			Marker: marker,
			MaxItems: 20
		};

		const res = await retry(
			(bail) => lambda
				.listAliases(params)
				.promise()
				.catch(bailIfErrorNotRetryable(bail)),
			getRetryConfig((err) => {
				console.warn("retrying listAliases after error...", { function: funcArn }, err);
			}));
		const versions = _.flatMap(res.Aliases, alias => {
			const versions = [alias.FunctionVersion];
			if (alias.RoutingConfig) {
				const additionalVersions = Object.keys(alias.RoutingConfig.AdditionalVersionWeights);
				return versions.concat(additionalVersions);
			} else {
				return versions;
			}
		});
		const newAcc = acc.concat(versions);

		if (res.NextMarker) {
			return loop(newAcc, res.NextMarker);
		} else {
			const uniqueVersions = _.uniq(newAcc);
			console.debug("found aliased versions", { 
				count: versions.length, 
				versions: uniqueVersions.join(",") 
			});
			return uniqueVersions;
		}
	};

	return loop();
};

/**
 * Delete version of the lambda
 * @param funcArn Lambda Function Arn
 * @param version Version of the lambda
 */
const deleteVersion = async (funcArn, version) => {
	console.info("deleting...", { function: funcArn, version });

	const params = {
		FunctionName: funcArn,
		Qualifier: version
	};

	await retry(
		(bail) => lambda
			.deleteFunction(params)
			.promise()
			.catch(bailIfErrorNotRetryable(bail)),
		getRetryConfig((err) => {
			console.warn("retrying deleteFunction after error...", { function: funcArn, version }, err);
		}));
};

module.exports = {
	listFunctions,
	listVersions,
	listAliasedVersions,
	deleteVersion
};